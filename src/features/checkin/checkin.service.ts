...
export class CheckinService {
  ...
  private async performCheckin(
    ...
    const ticket = await this.checkinRepository.findTicketForCheckin(
      registrationId,
      eventId,
    );
    if (!ticket) {
    ...
    if (ticket.status === "CANCELLED" || ticket.status === "EXPIRED") {
      throw new AppError(
        `Vé không hợp lệ. Trạng thái hiện tại: ${ticket.status}`,
        400,
      );
    }

    // 2. Cập nhật trạng thái vé thành CHECKED_IN
    const updatedTicket = await this.checkinRepository.markTicketAsCheckedIn(
      ticket._id.toString(),
    );
    if (!updatedTicket) {
      throw new AppError(
        "Đã xảy ra lỗi hệ thống khi cập nhật trạng thái vé.",
        500,
      );
    }
    ...
    try {
      const [totalCheckedIn, totalRegistered] = await Promise.all([
        this.checkinRepository.countCheckedIn(eventId),
        this.checkinRepository.countRegistered(eventId),
      ]);
      ...
      const attendee = updatedTicket.attendeeId as any;
